...
bool brother_black_sem_filho(No **raiz, No *subst){
    if(subst == NULL){
        return false;
    }
    No *brother = find_brother(subst);
    // Se NULL é PRETO, então ele é parte do caso que o irmao do elemento é preto?
    if(brother == NULL){
...
bool brother_RED(No **raiz, No *subst, bool subst_no_is_rem = false){
    if(subst == NULL){
        return false;
    }
    No *brother = find_brother(subst);
    if(subst_no_is_rem == false and brother->cor == VERMELHO){
    ...
    else if(subst_no_is_rem == true and brother->cor == VERMELHO){
        No *filho_esq = brother->left;
        Rot_simples_Esq(raiz, brother->father);
        remover_ele(raiz, subst);
...
void remocao(No **raiz, int num){
    ...
    if(remover != NULL){
        ...
        if(substituto_esq == remover->right){
                subst_no_is_rem = false;
        }

        if(substituto_esq == NULL){
            if(elemento_ROOT_sem_left_or_right(raiz, remover) == true){
                printf("Elem sem filho e é a raiz");
                return;
            }else{
        ...
        if(substituto_esq != NULL){
            ...
            if(ele_remov_RED(raiz, substituto_esq) == true){
                ...
                if(brother_black_sem_filho(raiz, substituto_esq) == true){
                    printf("\n\tIrmao preto sem filho");
                    associacao(raiz, substituto_esq, remover);
                    return;
                }else if(brother_black_with_1_filho_RED(raiz, substituto_esq, subst_no_is_rem) == true){
                    printf("\n\tIrmao preto com 1 filho RED");
                    associacao(raiz, substituto_esq, remover);
                    return;
                }else if(brother_RED(raiz, substituto_esq, subst_no_is_rem) == true){
                    printf("\n\tIrmao Vermelho");
                    associacao(raiz, substituto_esq, remover);
                    return;
                }
...
int main()
{
    ...
    for(int i = 0; i < tam; i++){
        NEW = crie(vetor[i]);
        inserir(&raiz, NEW);
        balancemento_sofredor(&raiz, NEW);
        verificacao(&raiz, raiz);
    }
